...
function VmMetricsCard({ vm, index }) {
  ...
  const fetch_ = useCallback(async () => {
    const name = vm.name;
    const [
      cpuUsagePct, cpuMhz, cpuMax, cpuDemand, cpuIdle, cpuReady,
    ...
    // CPU très précis comme ESXi
    const cpuPct = Math.min(firstVal(cpuUsagePct, 0), 100);
    ...
    setData({ 
      cpuPct, cpuMhzVal, cpuMaxMhz, cpuDemandVal, cpuIdleVal, cpuReadyVal,
      memPct, memConsumedKB, memMaxMB, memActiveMB, memSwappedKB,
      diskPct, diskFreeB, diskCapB, diskReadKB, diskWriteKB, diskLatMs,
      netRxKB, netTxKB, netDropR, netDropT, isOn, toolsOk, uptimeH 
    });
    setLoading(false);

    // Historique 30 min (corrigé)
    const [hCpuPct, hMem, hNetRx, hNetTx] = await Promise.all([
      pqRange(`vmware_vm_cpu_usage_average{vm_name="${name}"}`),
    ...
    const cpuHist = hCpuPct.map(v => Math.min(v, 100));
